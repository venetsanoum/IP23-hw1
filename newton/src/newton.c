...
//Συνάρτηση για τη δημιουργία του f(x).
double polynomial (double a0, double a1, double a2, double a3, double a4, double a5, double x) { 
   
 return a5 * pow(x,5) + a4 * pow(x,4) + a3 * pow(x,3) +a2 * pow(x,2) +a1 * pow(x,1) +a0 ;
}

//Συνάρτηση για τη δημιουργία της f'(x).
double derivative ( double a1, double a2, double a3, double a4, double a5, double x) { 
...
//Συνάρτηση newton.
double newton (double a0, double a1, double a2, double a3, double a4, double a5, double root) {
   ...
   double derivativefx = derivative (a1, a2, a3, a4, a5,root); //Υπολογισμός της τιμής της f'(x) για x=root.   
    ...
    int loopCount = 0; //Αρχικοποίηση του μετρητή των επαναλήψεων.
     while (loopCount < 1000) { //Όσο δεν έχουν ξεπεραστεί οι 1000 επαναλήψεις.
        ...
        if (derivativefx == 0 || isinf(root) || isnanf(root)) { //Αν f'(root) είναι μηδέν ή η ρίζα ειναι infinite ή not a number 
             printf("nan\n");                                   //τότε ο αλγόριθμος αποκλίνει και εκτυπώνεται "nan". 
            return 0;                                                     
