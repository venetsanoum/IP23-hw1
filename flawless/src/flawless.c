...
int perfectsquare(unsigned long long n) {  //Συνάρτηση που ελέγχει αν ενάς
                                           //αριθμός είναι τέλειο τετράγωνο.
  unsigned long root = (unsigned long)(sqrt(n));
  return root * root == n;  //Επιστρέφει αν το τετράγωνο της ρίζας ενός αριθμού
...
unsigned long long doubledigits(unsigned long long n) {
  ...
  unsigned long long restDigits =
      n % 100;  //Τα υπολοιπα ψηφία ειναι το υπολοιπο της διαιρεσης με το 100
  int sum = firstTwoDigits;  //Το αθροισμα ειναι αρχικα τα 2 πρωτα ψηφία

  while (restDigits >
         0) {  //Χωρισμος των υπολοιπων ψηφίων οπως στη συνάρτηση singledigits
...
int main(int argc, char **argv) {
  if (argc != 3) {
    printf(
        "Program needs to be called as: ./mirror lower upper\n");  //Αν ο
                                                                   //αριθμός των
                                                                   //ορισμάτων
                                                                   //δεν είναι
                                                                   //σωστός το
  ...
  if (lower > upper || lower < 1 ||
      upper > 1e15) {  //Εκτος ορίων περιπτώσεις τερματίζουν το πρόγραμμα με
                       //κωδικό εξόδου 1.
    return 1;
  }

  unsigned long long FlawlessSum = 0;  //Αρχικοποίηση του sum
  for (unsigned long i = (unsigned long)sqrt(lower);
       i <= (unsigned long)sqrt(upper); i++) {
    //Δημιουργία τετραγώνων που θα προσθεθουν στο sum(i = (long
    //double)sqrt(lower); i <= (long double)sqrt(upper);)
    if (perfectsquare(i * i)) {  //Έλεγχος αν το i*i είναι τέλειο τετράγωνο.
      int digitsum = singledigits(i * i);
      if ((i * i) <
          100) {  //Αν ο αριθμός είναι διψήφιος χωρίζεται σε μονα ψηφία
        if ((digitsum * digitsum) ==
            i * i) {  //Αν το αθροισμα των ψηφίων στο τετράγωνο είναι ισο με τον
                      //αριθμό τοτε ο αριθμός είναι αψογο τετράγωνο.
          FlawlessSum += i * i;
        }
...
int main (int argc, char **argv) {
    ...
    printf("%lld\n" ,FlawlessSum);
    return 0;
>>>>>>> b2b06f5236cf769ec9fe33defd1e65f79b5a8c22
}
